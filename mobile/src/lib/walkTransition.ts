...
// ── FATIGUE DE MONTÉE : la VAM tient-elle sur la durée ? ─────────────────────────
//
// Hypothèse à tester : le moteur apprend la vitesse ascensionnelle sur des côtes courtes
// à l'entraînement, jambes fraîches, puis l'applique à une course de trois heures avec
// 1 000 m de dénivelé cumulé. Si la VAM s'effondre avec le dénivelé déjà grimpé, cela
// suffit à expliquer un optimisme qui CROÎT avec la verticalité de la course — ce que le
// banc observe, et que ni l'altimétrie ni le découpage des pentes n'ont expliqué.
//
// ── Le piège méthodologique, et comment il est évité ──────────────────────────────
// La VAM dépend fortement de la pente. Comparer naïvement le début et la fin d'une sortie
// mesurerait donc le TERRAIN (souvent plus raide en altitude) et non la fatigue. On
// restreint donc la comparaison à une BANDE DE PENTE étroite : à pente comparable, toute
// baisse de VAM avec le dénivelé cumulé est imputable à l'athlète, pas au relief.

/** Bande de pente sur laquelle la fatigue est mesurée (%) — assez large pour avoir de la
 *  matière, assez étroite pour que la pente ne soit pas le facteur dominant. */
export const FATIGUE_GRADE_MIN_PCT = 10
export const FATIGUE_GRADE_MAX_PCT = 25

/** Largeur d'une tranche de dénivelé cumulé (m). */
export const FATIGUE_BIN_M = 250
/** Nombre de tranches (au-delà, tout est regroupé dans la dernière). */
export const FATIGUE_BIN_COUNT = 5

export interface ClimbFatigueBin {
  /** Borne basse du dénivelé déjà grimpé (m). */
  cumulativeGainMinM: number
  seconds: number
  meanVamMH: number | null
  meanSpeedKmH: number | null
  meanGradePct: number | null
  /** VAM rapportée à celle de la première tranche (1 = pas de perte). */
  vamRatioToFresh: number | null
}

/**
 * VAM par tranche de dénivelé déjà accumulé, À PENTE COMPARABLE.
 *
 * `meanGradePct` est publié pour que le lecteur puisse vérifier lui-même que les tranches
 * portent bien sur un terrain équivalent : si les pentes moyennes divergent nettement, la
 * comparaison ne vaut rien et le tableau doit être lu comme tel.
 */
export function aggregateByClimbFatigue(samples: Sample[]): ClimbFatigueBin[] {
  const acc = Array.from({ length: FATIGUE_BIN_COUNT }, (_, k) => ({
    cumulativeGainMinM: k * FATIGUE_BIN_M,
    seconds: 0, vamWeighted: 0, spdWeighted: 0, gradeWeighted: 0,
  }))

  for (const s of samples) {
    if (s.gradePct < FATIGUE_GRADE_MIN_PCT || s.gradePct > FATIGUE_GRADE_MAX_PCT) continue
    const k = Math.min(FATIGUE_BIN_COUNT - 1, Math.floor(s.cumulativeGainM / FATIGUE_BIN_M))
    const b = acc[k]
    if (!b) continue
    b.seconds += s.seconds
    b.vamWeighted += s.speedKmH * 1000 * (s.gradePct / 100) * s.seconds
    b.spdWeighted += s.speedKmH * s.seconds
    b.gradeWeighted += s.gradePct * s.seconds
  }

  const bins: ClimbFatigueBin[] = acc.map((b) => ({
    cumulativeGainMinM: b.cumulativeGainMinM,
    seconds: Math.round(b.seconds),
    meanVamMH: b.seconds > 0 ? Math.round(b.vamWeighted / b.seconds) : null,
    meanSpeedKmH: b.seconds > 0 ? +(b.spdWeighted / b.seconds).toFixed(2) : null,
    meanGradePct: b.seconds > 0 ? +(b.gradeWeighted / b.seconds).toFixed(1) : null,
    vamRatioToFresh: null,
  }))

  // Référence = première tranche EXPLOITABLE (≥ 5 min), pas forcément la toute première.
  const fresh = bins.find((b) => b.seconds >= 300 && b.meanVamMH != null)
  if (fresh?.meanVamMH) {
    for (const b of bins) {
      if (b.meanVamMH != null && b.seconds >= 300) {
        b.vamRatioToFresh = +(b.meanVamMH / fresh.meanVamMH).toFixed(3)
      }
    }
  }
  return bins
...
/** Chaîne complète : streams d'un athlète → fatigue de montée. */
export function measureClimbFatigue(streamSets: WalkTransitionStreams[]): ClimbFatigueBin[] {
  const samples: Sample[] = []
  for (const s of streamSets) samples.push(...extractSamples(s))
