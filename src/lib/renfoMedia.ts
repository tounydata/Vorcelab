// Démos d'exercices — WebP animés (GymVisual, licence commerciale royalty-free),
// ré-hébergés en local dans public/exercises/<id>.webp et servis par le CDN Pages.
// Un id absent de COVERED retombe sur le placeholder SVG animé (ExerciseMedia).
// Voir docs/exercise-media.md.

const BASE = `${import.meta.env.BASE_URL}exercises`

// Tous les exercices actifs du catalogue ont une démo WebP.
...
// Exercices ayant AUSSI une démo « maison » distincte (poids de corps / haltère /
// bande) en plus de la version salle : <id>.maison.webp. Utilisé quand le lieu
// sélectionné dans la séance est « maison ».
const HAS_MAISON = new Set<string>([
  'squat_lourd', 'rdl', 'bulgare', 'mollets_lourds', 'hip_thrust',
])

export type ExoLocation = 'maison' | 'salle' | undefined

/** URL de la démo WebP d'un exercice (ou null → placeholder). */
