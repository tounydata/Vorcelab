...
import { buildRenfoRows, isRenfo, type StravaActLite, type ExistingRenfoRow } from './renfoBackfill'
...
export async function syncStravaRenfo(userId: string, sinceDays = 90): Promise<number> {
  ...
  const { data: acts, error: aErr } = await supabase
    ...
    .select('strava_activity_id,type,sport_type,start_date,start_date_local,moving_time,raw_data')
    .gte('start_date_local', cutoffISO)
  if (aErr || !acts) return 0
  if (!acts.some((a) => isRenfo(a.type, a.sport_type))) return 0

  // Déduplication par id d'activité Strava : deux séances le même jour issues de
  // deux activités différentes sont conservées. On lit aussi source_activity_id
  // pour ne pas ré-importer une séance déjà présente.
  const { data: existing } = await supabase
    .from('renfo_session_log')
    .select('session_date,focus,source_activity_id')
    .eq('user_id', userId)
