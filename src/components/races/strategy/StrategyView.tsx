...
export default function StrategyView({ projection: p, race, athleteName, nutritionRows, ravitos, forecast, weather }: Props) {
  ...
  const route = useMemo(() => buildRoutePath(p.points, totalKm), [p.points, totalKm])
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        ...
        <div className="strat-hero-grid" style={{ padding: '6px 18px 12px', display: 'flex', gap: 16, alignItems: 'stretch' }}>
          ...
          {route && <div className="strat-hero-map" style={{ flex: '0 0 290px' }}><RouteMap route={route} markers={markers} cursorKm={hoverKm} totalKm={totalKm} heightPx={300} /></div>}
        </div>
...
function RouteMap({ route, markers, cursorKm, totalKm, heightPx }: {
  route: { d: string; project: (km: number) => { x: number; y: number } | null }
  markers: ProfileMarker[]; cursorKm: number | null; totalKm: number; heightPx: number
}) {
  const cur = cursorKm != null ? route.project(cursorKm) : null
  ...
  return (
    <div style={{ background: 'var(--vl-surf)', border: '1px solid var(--vl-line)', borderRadius: 'var(--vl-r)', overflow: 'hidden', display: 'flex', flexDirection: 'column', height: heightPx }}>
      ...
      <div style={{ position: 'relative', flex: 1, minHeight: 0, margin: '0 12px 12px', borderRadius: 'var(--vl-r-sm)', overflow: 'hidden', background: 'color-mix(in srgb, var(--vl-surf-2) 70%, var(--vl-bg))' }}>
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
          <path d={route.d} fill="none" stroke="var(--vl-ember)" strokeOpacity={0.25} strokeWidth={6} strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" style={{ filter: 'blur(1px)' }} />
          <path d={route.d} fill="none" stroke="var(--vl-ember)" strokeWidth={2.2} strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" />
        </svg>
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
          {mk.map((m, i) => {
            const pt = route.project(m.km); if (!pt) return null
            const ring = m.kind === 'start' || m.kind === 'finish'
            const c = m.kind === 'finish' ? 'var(--vl-growth-2)' : m.kind === 'start' ? 'var(--vl-ember)' : 'var(--vl-text)'
            return (
              <div key={i} style={{ position: 'absolute', left: pt.x + '%', top: pt.y + '%', transform: 'translate(-50%,-50%)', display: 'flex', alignItems: 'center', gap: 4 }}>
                <span style={{ width: ring ? 10 : 8, height: ring ? 10 : 8, borderRadius: 999, background: c, border: '2px solid var(--vl-surf)', boxShadow: ring ? `0 0 0 3px color-mix(in srgb, ${c} 26%, transparent)` : '0 1px 3px rgba(0,0,0,.5)' }} />
                <span className="mono" style={{ fontSize: 8.5, fontWeight: 700, color: c, textShadow: '0 1px 2px var(--vl-bg)', letterSpacing: '.04em' }}>{m.kind === 'start' ? 'DÉP' : m.kind === 'finish' ? 'ARR' : 'R' + m.km}</span>
              </div>
            )
          })}
          {cur && <div style={{ position: 'absolute', left: cur.x + '%', top: cur.y + '%', transform: 'translate(-50%,-50%)', width: 14, height: 14, borderRadius: 999, background: 'var(--vl-growth-2)', border: '2px solid var(--vl-bg)', boxShadow: '0 0 0 5px color-mix(in srgb, var(--vl-growth-2) 28%, transparent)' }} />}
        </div>
        <div style={{ position: 'absolute', top: 8, right: 9, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
          <span className="mono" style={{ fontSize: 8, color: 'var(--vl-text-2)', fontWeight: 700 }}>N</span>
